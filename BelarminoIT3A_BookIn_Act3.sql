...
INSERT INTO `author` VALUES ('Pablo Neruda',1),('Franklin D. Lewis',2),('Kristina Mahr',3),('Noor Shirazie',4),('Margaret Atwood',5),('Michael Faudet',6),('John Keats',7),('Mary Oliver',8),('Allen Gisnberg',9),('Trista Mateer',10),('Atticus',11);
...
INSERT INTO `book` VALUES (1234567880,'Love Poems',2008,525,70,1,10),(1234567890,'Rumi',2003,100,5,2,1),(1234567891,'Say It\'s the Sea',2020,200,10,3,2),(1234567892,'Into th Wildfire',2016,300,15,4,NULL),(1234567893,'In Other Worlds',2011,400,50,5,3),(1234567894,'Smoke & Mirrors',2017,400,30,6,4),(1234567895,'Complete Poems',2009,210,20,7,5),(1234567896,'Felicity',2015,500,25,8,6),(1234567897,'Howl and Other Poems',1979,255,35,9,7),(1234567898,'Honeybee',2018,560,40,10,8),(1234567899,'The Dark Between Stars',2018,600,65,11,9);
...
CREATE TABLE `order` (
  ...
  `ORDER_FILLED` tinyint(1) NOT NULL,
...
INSERT INTO `order` VALUES (1,1,'2023-01-01',1),(2,2,'2023-01-02',1),(3,3,'2023-01-03',1),(4,4,'2023-01-06',1),(5,5,'2023-01-06',1),(6,6,'2023-01-08',1),(7,7,'2023-01-12',1),(8,8,'2023-01-18',1),(9,9,'2023-01-25',1),(10,10,'2023-02-02',1),(11,1,'2023-02-06',1),(12,2,'2023-02-14',1),(13,3,'2023-02-25',0);
...
/*!50003 CREATE*/ /*!50017 DEFINER=`root`@`localhost`*/ /*!50003 TRIGGER `UPDATE_TOTAL_SPENT2` AFTER UPDATE ON `order` FOR EACH ROW BEGIN
	-- Add the order's cost to customer's total_spent when the order is paid.
...
CREATE TABLE `order_line` (
  ...
  CONSTRAINT `order_line_ibfk_2` FOREIGN KEY (`ORDER_ID`) REFERENCES `order` (`ORDER_ID`) ON DELETE CASCADE
...
INSERT INTO `order_line` VALUES (1,1234567890,1,100,100,1),(2,1234567891,1,200,400,2),(3,1234567892,2,300,900,3),(4,1234567893,3,400,400,1),(5,1234567894,4,400,800,2),(6,1234567895,5,210,630,3),(7,1234567896,6,500,2000,4),(8,1234567897,7,255,255,1),(9,1234567898,7,560,1120,2),(10,1234567899,8,600,1800,3),(11,1234567880,8,525,2100,4),(12,1234567890,9,100,100,1),(13,1234567891,10,100,100,1),(14,1234567896,11,500,500,1),(15,1234567896,12,500,1000,2),(16,1234567896,13,500,500,1);
...
/*!50003 CREATE*/ /*!50017 DEFINER=`root`@`localhost`*/ /*!50003 TRIGGER `UPDATE_TOTAL_SPENT` AFTER INSERT ON `order_line` FOR EACH ROW BEGIN
	-- Update the customer's total_spent when they order another book (new order_line).
...
/*!50003 CREATE*/ /*!50017 DEFINER=`root`@`localhost`*/ /*!50003 TRIGGER `DEL_ORDER` BEFORE DELETE ON `order_line` FOR EACH ROW BEGIN
	-- If the deleted order_line is the last one referencing the order_id, delete the order_id row from the order table as well (because it is an empty order).
...
INSERT INTO `publisher` VALUES ('Oneworld Publications',1),('Independent',2),('Knopf Doubleday Publishing Group',3),('Andrews McMeel Publishing',4),('Random House Publishing Group',5),('Penguin Press',6),('City Lights Books',7),('Central Avenue Publishing',8),('Atria Books',9),('New Directions',10);
...
-- Dump completed on 2023-02-25 21:20:10
